...
struct trapframe {
	...
	u_int32_t tf_v0;	/* Saved register 2 (v0), stores the return value if no error, or the error code if error */
	u_int32_t tf_v1;	/* etc. */
	u_int32_t tf_a0;
	u_int32_t tf_a1;
	u_int32_t tf_a2;
	u_int32_t tf_a3;	/* binary bit to signal if error occurred, 0 for no error, 1 for error */
	u_int32_t tf_t0;
	u_int32_t tf_t1;
	u_int32_t tf_t2;
	u_int32_t tf_t3;
	...
	u_int32_t tf_sp;
	u_int32_t tf_s8;
	u_int32_t tf_epc;	/* coprocessor 0 epc register, program counter */
};
